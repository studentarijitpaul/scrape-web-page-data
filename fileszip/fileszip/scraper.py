...
Google Sheets setup (one-time):
    ...
    3. Add the JSON key as a Replit Secret named GOOGLE_SERVICE_ACCOUNT_JSON.
    4. Set GOOGLE_SHEET_ID to your spreadsheet's ID (from its URL).
...
def save_to_google_sheet(rows: list[dict], month_label: str) -> None:
    ...
    Requires two environment variables / secrets:
    ...
    if not sa_json:
        print("\n⚠  GOOGLE_SERVICE_ACCOUNT_JSON not set — skipping Google Sheets upload.")
        return
    if not sheet_id:
        print("\n⚠  GOOGLE_SHEET_ID not set — skipping Google Sheets upload.")
        return
