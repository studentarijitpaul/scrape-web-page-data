...
def send_new_exam_message(row: dict) -> bool:
    return _post_message(
        "🆕 New Exam Added\n\n"
        f"Exam: {row.get('exam', '')}\n"
...
def send_update_message(previous: dict, current: dict) -> bool:
    changed = []
    for field, label in (("date", "Date"), ("event", "Event"), ("event_type", "Event type"), ("exam_url", "Exam URL")):
        if str(previous.get(field, "")).strip() != str(current.get(field, "")).strip():
            old = _format_date(previous.get(field, "")) if field == "date" else previous.get(field, "")
            new = _format_date(current.get(field, "")) if field == "date" else current.get(field, "")
            changed.append(f"{label}: {old} → {new}")
    heading = "🔄 Exam Date Changed" if set(previous) and previous.get("date") != current.get("date") else "🔄 Exam Information Updated"
    return _post_message(f"{heading}\n\nExam: {current.get('exam', '')}\n\nChanged:\n" + "\n".join(changed) + "\n\nSource: Shiksha")
