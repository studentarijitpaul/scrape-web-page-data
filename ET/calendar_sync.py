...
# Google Calendar custom event IDs must contain only lowercase a-v and 0-9.
# "exam" is INVALID as a prefix because it contains 'x' (outside a-v).
EVENT_ID_PREFIX = "evt"
...
def make_event_id(exam: str, date_str: str, event_type: str) -> str:
    ...
    # Google Calendar custom event IDs must match [a-v0-9]{5,1024} — validate
    # here so a bad EVENT_ID_PREFIX fails loudly at the first row instead of
    # producing a cryptic 400 "Invalid resource id value" per-row later.
    if not re.fullmatch(r"[a-v0-9]{5,1024}", event_id):
        raise ValueError(f"Generated invalid Google Calendar event ID: {event_id!r}")
    return event_id
