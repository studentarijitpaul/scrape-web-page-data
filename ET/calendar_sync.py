...
from change_detector import normalize_exam_name
import google_chat
...
def make_event_id(
    ...
    key = (
        f"{normalize_exam_name(exam)}|"
        f"{normalize_exam_name(event_type)}"
    )
...
def list_existing_synced_events(
    ...
    while True:
        ...
        for item in response.get(
            ...
            event_id = item.get("extendedProperties", {}).get("private", {}).get("row_key") or item.get("id")
...
def run_sync(rows: Optional[list[dict]] = None, tab_name: Optional[str] = None) -> SyncStats:
